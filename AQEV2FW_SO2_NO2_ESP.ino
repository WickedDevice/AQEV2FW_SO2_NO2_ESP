...
#define EEPROM_SO2_BASELINE_VOLTAGE_TABLE (EEPROM_NO2_BASELINE_VOLTAGE_TABLE - (5*sizeof(baseline_voltage_t))) // array of (up to) five structures for baseline offset characterization over temperature
...
#define EEPROM_SO2_SENSITIVITY    (EEPROM_PM10P0_CAL_OFFSET - 4)      // float value, 4-bytes, the sensitivity from the sticker  [UNUSED]
...
#define EEPROM_BACKUP_SO2_SENSITIVITY    (EEPROM_BACKUP_NO2_CAL_OFFSET + 4)
...
#define BACKUP_STATUS_SO2_CALIBRATION_BIT         (4)
...
void initializeHardware(void) {
    ...
    Serial.println(F(" |       SO2 / NO2 Sensor Suite       |"));
...
// only apply it if the slope is not already self consistent with the sensitivity
    ...
    calculated_slope = convert_so2_sensitivity_to_slope(sensitivity);
...
void restore(char * arg) {
    ...
    if (strncmp(arg, "defaults", 8) == 0) {
        ...
        eeprom_write_block("aqev2_so2_no2_esp", (void *) EEPROM_UPDATE_FILENAME, 32);
    }
...
boolean publishHeartbeat() {
    ...
    snprintf(scratch, SCRATCH_BUFFER_SIZE-1,
             ...
             "\"converted-value\":%d,"
             "\"firmware-version\":\"%s\","
             "\"publishes\":[\"particulate\",\"so2\",\"no2\",\"temperature\",\"humidity\"%s],"
             "\"counter\":%lu"
